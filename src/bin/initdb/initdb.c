...
bootstrap_template1(void)
{
	...
	snprintf(cmd, sizeof(cmd),
			 "\"%s\" --boot -X %d %s %s %s %s %s %s %s %s %s %s %s %s",
			 backend_exec,
...
usage(const char *progname)
{
	...
	printf(_("  -C, --extended-checksums={32|64}\n"
...
main(int argc, char *argv[])
{
	static struct option long_options[] = {
		...
		{"extended-checksums", required_argument, NULL, 'C'},
	...
	while ((c = getopt_long(argc, argv, "A:c:C:dD:E:gkK:L:nNRsST:u:U:WwX:", long_options, &option_index)) != -1)
	{
		switch (c)
		{
			case 'A':
