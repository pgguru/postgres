...
my $node =  PostgreSQL::Test::Cluster->new('node_a');
...
$node->safe_psql('postgres', "SELECT 'init' FROM pg_create_physical_replication_slot('regress_pg_waldump_slot', true, false);
...
$node->command_ok(['pg_waldump', '--save-fullpage', "$tmp_folder/raw", '--relation', $relation, $walfile]);

# This regexp will match filenames of the format XXXXXXXX-XXXXXXXX.dd.dd.dd.dd_fork
# with the components being:
# - WAL LSN in hex format,
# - Decimal database oid
# - Decimal tablespace oid or 0 for global
# - Decimal relfilenumber
# - Decimal block number
# - fork this block came from (one of vm, init, fsm, or main)
my $file_re =
...
ok($file_count > 0, 'verify we processed some files');
